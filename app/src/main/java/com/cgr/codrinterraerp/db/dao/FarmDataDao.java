...
public interface FarmDataDao {
    ...
    @Query("SELECT r.circumference, r.length, r.pieces, r.grossVolume, r.netVolume, r.tempFarmDataId, " +
            "r.farmDataId, r.tempFarmId, r.thickness, r.width, r.volumePie " +
            "FROM farm_data r " +
            "JOIN farm_details d ON d.tempFarmId = r.tempFarmId " +
            "WHERE r.tempFarmId = :tempFarmId AND r.isDeleted = 0 AND d.isDeleted = 0 ORDER BY r.createdAt DESC")
    LiveData<List<FarmCapturedData>> fetchByTempFarmId(String tempFarmId);
    ...
    @Query("SELECT 0 as updatedAt, tempFarmId as tempFarmId, " +
            "IFNULL(SUM(pieces),0) AS totalPieces, " +
            "IFNULL(SUM(grossVolume),0) AS totalGrossVolume, " +
            "IFNULL(SUM(netVolume),0) AS totalNetVolume, " +
            "IFNULL(SUM(volumePie),0) AS totalVolumePie " +
            "FROM farm_data " +
            "WHERE tempFarmId = :tempFarmId AND isDeleted = 0")
    LiveData<FarmSummary> getSummaryByTempId(String tempFarmId);
}